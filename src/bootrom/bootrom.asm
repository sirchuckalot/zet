;
;  Zet PC system BOOTROM helper functions in 8086 assembly
...
;;  Bootrom memory 
;;--------------------------------------------------------------------------
BOOTMEM_ROMBASE                 equ     0xFE00  ;; We are exactly 4kb in size
BOOTMEM_STACKSEG                equ     0x1000
BOOTMEM_DATASEG                 equ     0x2000
...
_BOOTSEG                SEGMENT 'CODE'
                        assume  cs:_BOOTSEG
;;--------------------------------------------------------------------------
bootrom:                org     0x0000         ;; start of ROM, get placed at 00000h
                        db      0x55, 0xaa     ;; BIOS signature, required for BIOS extensions 
                        db      0x40           ;; BIOS extension length in units of 512 bytes 
bootrom_entry_point:    jmp     bootrom_init_func
;;--------------------------------------------------------------------------
...
;;--------------------------------------------------------------------------
;;--------------------------------------------------------------------------
bootrom_init_func:      push ds                       ; Make sure we can return back to 1st stage caller
                        push es
                        push ss
                        push sp
                        
                        POSTCODE 002h                 ; PostCode = 02
                        call    init_text             ; init video controller for text mode output
                        
                        POSTCODE 003h                 ; PostCode = 03
                        call    _sd_card_bios_loader  ; Call 2nd stage loader
                        
                        pop sp                        ; return back to 1st stage bootrom
                        pop ss
                        pop es
                        pop ds
...
;;--------------------------------------------------------------------------
;;--------------------------------------------------------------------------
;;  Init video controller for text mode output:
;;--------------------------------------------------------------------------
;;--------------------------------------------------------------------------
init_text:
...
;;           SET_INT_VECTOR 0x10, 0xC000, vgabios_int10_handler ;; set int10 vect
                        ...
                        call    vgabios_int10_handler       ;; Equivalant to int 0x10 bios service
...
int10_normal:           push    es
                        ...
                        mov     bx, BOOTMEM_ROMBASE   ;; We have to set ds to access the right data segment
                        mov     ds, bx
                        ...
                        pop     es
int10_end:              popf
                        ret                  ;; We normally would execute and iret from here
...
;;  Tell who we are
;;--------------------------------------------------------------------------
display_info:           mov     ax, BOOTMEM_ROMBASE
                        mov     ds, ax
